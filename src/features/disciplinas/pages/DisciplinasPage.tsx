...
export default function DisciplinasPage() {
    const { disciplinas, loading, error, createDisciplina, deleteDisciplina, updateDisciplina } = useDisciplinas()
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        const res = await createDisciplina(novaDisciplina.trim())
        if (res.success) setNovaDisciplina('')
    ...
    const handleSaveEdit = async (id: string) => {
        ...
        const res = await updateDisciplina(id, editName.trim())
        if (res.success) {
            setEditingId(null)
            setEditName('')
        }
    ...
    if (error) {
        return (
            <div className="flex h-64 flex-col items-center justify-center text-center">
                ...
                <h3 className="text-lg font-medium text-text-primary">Erro ao carregar disciplinas</h3>
            ...
            <div className="mx-auto max-w-4xl space-y-8">
                    ...
                    <div className="flex items-start sm:items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="mt-2 text-sm text-text-muted">
                                Gerencie a grade de matérias oferecidas por sua instituição.
                            </p>
                        </div>
                    </div>
                </header>
                ...
                <main className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                    ...
                    <div className="card p-6 md:col-span-1 border border-border-subtle shadow-lg">
                        <h2 className="text-lg font-semibold text-text-primary mb-4">Nova Disciplina</h2>
                    ...
                    <div className="card md:col-span-2 overflow-hidden border border-border-subtle shadow-lg">
                        {disciplinas.length === 0 ? (
                            <div className="flex flex-col items-center justify-center p-12 text-center">
                                ...
                                <h3 className="text-lg font-medium text-text-primary">Nenhuma disciplina cadastrada</h3>
                                <p className="mt-2 text-sm text-text-muted max-w-sm">
                                    A base de disciplinas está vazia. Comece adicionando matérias usando o painel lateral.
                                </p>
                            </div>
                        ) : (
                            <ul className="divide-y divide-border-subtle">
                                {disciplinas.map((disciplina) => (
                                    <li key={disciplina.id} className="flex items-center justify-between p-4 hover:bg-surface-raised transition-colors group">
                                        {editingId === disciplina.id ? (
                                            <div className="flex items-center gap-3 w-full">
                                                ...
                                                />
                                                <button
                                                    onClick={() => handleSaveEdit(disciplina.id)}
                                            ...
                                            <>
                                                <div className="flex items-center gap-3">
                                                    <div
                                                        className="h-8 w-8 rounded-full flex items-center justify-center font-bold text-xs ring-1 ring-black/5"
                                                        style={{
                                                            backgroundColor: disciplina.cor?.cor_bg || '#f3f4f6',
                                                            color: disciplina.cor?.cor_text || '#374151'
                                                        }}
                                                    >
                                                        {disciplina.nome.substring(0, 2).toUpperCase()}
                                                    </div>
                                                    <span className="font-medium text-text-primary">{disciplina.nome}</span>
                                                </div>
                                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                                                    <button
                                                        onClick={() => handleStartEdit(disciplina.id, disciplina.nome)}
